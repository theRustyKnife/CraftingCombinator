...
-- attempts to find the best suitable icon for this recipe - uses a default one if none is found
	...
	if recipe.icon then return {{icon = recipe.icon}}; end                                                                                                -- the recipe has it's icon
	if recipe.icons then return recipe.icons; end
	for _, type in pairs({"item", "module", "tool", "fluid"}) do
		if recipe.result and data.raw[type][recipe.result] and data.raw[type][recipe.result].icon then return {{icon = data.raw[type][recipe.result].icon}}; end -- the result has an icon
		if recipe.result and data.raw[type][recipe.result] and data.raw[type][recipe.result].icons then return data.raw[type][recipe.result].icons; end
	...
	return {{icon = "__crafting_combinator__/graphics/no-icon.png"}}-- no icon found - use the default one
end
